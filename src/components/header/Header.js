...
const Header = () => {
  const [length, setLength] = useState(0); // Initialize length state with 0
  const token = sessionStorage.getItem('token')

  useEffect(() => {
    updateCartItemCount(); // Call the function to update cart item count on component mount
  }, []); // Run this effect only once on component mount

  const updateCartItemCount = () => {
    // Retrieve cart items from sessionStorage and update the length state
  ...
  const navigation = {
    ...
    //         items: [
    //           { name: 'CASSEROLE', href: '#' },
    //           { name: '3 IN 1 COOKER', href: '#'},
    //           { name: 'CO', href: '#'},
    //           { name: 'DEEP PAN PRESSURE COOKER', href: '#'},
    //           { name: 'OUTER LID', href: '#'},
    //           { name: 'INNER LID', href: '#'},
    //           { name: 'SAS MITAL PRESSURE COOKERS', href: '#'},
    //           { name: 'COMBO SETS', href: '#'},
  ...
  return (
    <div className="bg-white">
      ...
      <header className="relative z-10 bg-white">
        <p className="flex h-10 items-center justify-center bg-danger px-4 text-sm font-medium text-white sm:px-6 lg:px-8">
          WELCOME TO CAMRO - STAINLESS STEEL WORLD
        </p>
            ...
            <div className="flex h-16 items-center">
              ...
              <div className="ml-auto flex items-center">
                <div className="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
                  ...
                  <Link onClick={handleLogout} className="text-sm font-medium text-gray-700 hover:text-gray-800">
                    Logout
                  </Link>
                ...
                <div className="ml-4 flow-root lg:ml-6">
                  <Link to="javascript:void(0)" className="group -m-2 flex items-center p-2">
                    <ShoppingBagIcon
                      onClick={handleCartOpen}
                      className="h-6 w-6 flex-shrink-0 text-gray-400 group-hover:text-gray-500"
                      aria-hidden="true"
                    />
                    {/* <span className="ml-2 text-sm font-medium text-gray-700 group-hover:text-gray-800">{length}</span>                    <span className="sr-only">items in cart, view bag</span> */}
                  </Link>
                </div>
              </div>
            </div>
